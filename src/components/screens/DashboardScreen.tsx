...
import { Target, Zap, Clock, ShieldAlert, Sparkles, Activity, ArrowRight, Flame, Trophy } from "lucide-react";
...
export function DashboardScreen() {
  ...
  return (
    <div className="w-full max-w-full overflow-x-hidden min-h-0">
      ...
      <div className="block md:hidden p-4 space-y-4 w-full overflow-x-hidden">
        ...
        <div className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 flex flex-col gap-3 backdrop-blur-md">
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="text-xl font-black text-white">{user.xp} <span className="text-xs text-slate-400 font-bold uppercase tracking-widest">XP</span></div>
            </div>
            <div className="relative w-10 h-10 flex items-center justify-center">
              <svg className="w-full h-full transform -rotate-90">
                <circle cx="20" cy="20" r="16" className="stroke-white/10" strokeWidth="4" fill="none" />
                <circle cx="20" cy="20" r="16" className="stroke-amber-400" strokeWidth="4" fill="none" strokeDasharray="100" strokeDashoffset={100 - user.levelProgress} strokeLinecap="round" />
              </svg>
              <span className="absolute text-[10px] font-bold">{user.level}</span>
            </div>
          </div>
