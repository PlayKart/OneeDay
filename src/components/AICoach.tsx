...
export const AICoach = () => {
  ...
  return (
    <div className="flex flex-col h-[550px] md:h-[600px] overflow-hidden bg-[#07070a]/40 backdrop-blur-2xl">
      {/* Header Panel */}
      <div className="p-4 border-b border-white/[0.05] bg-white/[0.01] flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2.5">
          <div className="w-6 h-6 rounded-lg bg-gradient-to-tr from-blue-500 via-violet-500 to-pink-500 p-[1px]">
          ...
          <div>
            ...
            <span className="text-[8px] font-bold text-violet-400 uppercase tracking-widest block leading-none">Intelligence Coach</span>
          </div>
        </div>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 scrollbar-hide pb-28">
        <AnimatePresence initial={false}>
          {messages.map((msg, i) => (
            ...
            >
              {msg.role !== 'user' && (
                <div className="w-7 h-7 rounded-full bg-gradient-to-tr from-[#3b82f6] via-[#8b5cf6] to-[#ec4899] p-[1px] shrink-0 shadow-[0_0_12px_rgba(139,92,246,0.15)]">
              ...
              <div className={`max-w-[85%] text-sm leading-relaxed ${
                msg.role === 'user' 
                  ? 'bg-white/[0.04] border border-white/5 rounded-[1.25rem] rounded-tr-sm px-4 py-3 text-slate-200 shadow-md font-sans' 
        ...
        {loading && (
          ...
          >
            <div className="w-7 h-7 rounded-full bg-gradient-to-tr from-[#3b82f6] via-[#8b5cf6] to-[#ec4899] p-[1px] shrink-0 animate-spin-slow">
              <div className="w-full h-full rounded-full bg-[#08080c] flex items-center justify-center">
                <Sparkles size={11} className="text-violet-400" />
              </div>
            </div>
            <div className="flex-1 space-y-2 max-w-sm pt-2">
              <div className="h-1.5 w-full bg-gradient-to-r from-[#3b82f6] via-[#8b5cf6] to-[#ec4899] rounded-full animate-pulse opacity-40" />
              <div className="h-1.5 w-[75%] bg-gradient-to-r from-[#3b82f6] via-[#8b5cf6] to-[#ec4899] rounded-full animate-pulse opacity-30" />
            </div>
          </motion.div>
        )}
      </div>
      ...
      <form onSubmit={handleSend} className="absolute bottom-0 inset-x-0 p-4 bg-gradient-to-t from-[#030303] via-[#030303]/90 to-transparent pt-12">
        <div className="relative max-w-2xl mx-auto gemini-border-glow rounded-[1.75rem]">
          <div className="w-full bg-[#0c0c12]/80 border border-white/[0.06] rounded-[1.75rem] px-5 py-3 flex items-center gap-3 shadow-2xl">
